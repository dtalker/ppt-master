...
前者:把 `.pptx` 连同素材(或一个主题)给 AI,说「套模板」——见 [套模板工作流](../../skills/ppt-master-richard/workflows/template-fill-pptx.md)。本节其余部分讲 create-template。
...
| 位置 | 路径 | 说明 |
|---|---|---|
| **注册进 skill 库** | `skills/ppt-master-richard/templates/layouts/<id>/` | 全局,所有项目可复用;跑 `register_template.py` 后,问"有哪些模板"时会被列出来 |
| **放进项目里** | `projects/<project>/templates/` | 项目本地;给路径即用,无需注册 |

无论放哪,生成时都靠在对话里给出它的**目录路径**来引用——工作流只认显式路径,绝不认裸模板名:

```
你：用 sources/report.pdf 做 deck,模板用 skills/ppt-master-richard/templates/layouts/academic_defense/
```
...
完整说明 → [实时预览工作流](../../skills/ppt-master-richard/workflows/live-preview.md)
...
| 情况 | 先试这个 |
|---|---|
| AI 跑偏或漏了步骤 | 让它重新读 `skills/ppt-master-richard/SKILL.md`。 |
