...
```bash
python3 skills/ppt-master-richard/scripts/svg_to_pptx.py <project> -t push       # 换转场效果
python3 skills/ppt-master-richard/scripts/svg_to_pptx.py <project> -t none       # 关闭转场
python3 skills/ppt-master-richard/scripts/svg_to_pptx.py <project> -a none       # 关闭页内动画
python3 skills/ppt-master-richard/scripts/svg_to_pptx.py <project> --animation fade        # 改用单一效果（仍是默认级联）
python3 skills/ppt-master-richard/scripts/svg_to_pptx.py <project> --animation-trigger on-click   # 改为单击触发，演讲者控制节奏
```
...
输出仍是 100% 原生可编辑的 PowerPoint：原设计、母版、图片、动画都保留，且只导出选中的页面。它刻意**不**改版式、不加页、不换图——一份 deck 的页面结构本身承载着逻辑（总分、对比、递进），所以应挑选结构本就契合内容的页面，而不是硬塞进去。若需要全新结构或不同页数，请改用 create-template（见下一问）。完整步骤：[套模板工作流](../../skills/ppt-master-richard/workflows/template-fill-pptx.md)。
...
> 更多问题可先查看 [skills/ppt-master-richard/SKILL.md](../../skills/ppt-master-richard/SKILL.md) 与 [AGENTS.md](../../AGENTS.md)
